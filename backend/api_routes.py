...
class APIRoutes:
    ...
    def register_routes(self, blueprint):
        ...
        # Debug endpoint to get raw evaluation format
        @blueprint.route('/debug/evaluation-format', methods=['GET'])
        @login_required
        def get_evaluation_format():
            try:
                ...
                # Extract just the evaluation_result portion
                ...
                logger.error(f"Error getting evaluation format: {str(e)}")
                return jsonify({"error": str(e)}), 500

        # Dashboard data endpoint - combines evaluations, sessions, and stats
        ...
        def get_dashboard_data():
            try:
                ...
                # Get evaluations for stats and recent cases
                evaluations = list(mongo.db.evaluations.find(
                    {"user_id": user.user_id}
                ...
                # Process evaluations for stats
                if evaluations:
                    completed = len(evaluations)

                    # Calculate average diagnosis score instead of just counting correct/incorrect
                    total_score = 0
                    for eval in evaluations:
                        eval_result = eval.get('evaluation_result', {})

                        # Try to get the score in this order: overall_performance, diagnosis_accuracy_score, overall_clinical_score
                        score = eval_result.get('overall_performance')
                        if score is None:
                            score = eval_result.get('diagnosis_accuracy_score')
                        if score is None:
                            score = eval_result.get('overall_clinical_score')

                        if score is not None and isinstance(score, (int, float)):
                            total_score += float(score)

                    # Calculate average score (out of 10)
                    avg_score = round((total_score / completed), 1) if completed > 0 else 0

                    dashboard_data["stats"]["completedCases"] = completed
                    dashboard_data["stats"]["accuracyRate"] = avg_score

                # Process all cases (evaluations + in-progress sessions)
                recent_cases = []

                    # Process completed cases from evaluations
                for eval in evaluations[:10]:  # Limit to 10 most recent
                    case_data = eval.get('case_info', {})
                    evaluation_result = eval.get('evaluation_result', {})

                    # Get the diagnosis accuracy score if available
                    # First try overall_performance, then diagnosis_accuracy_score, then overall_clinical_score
                    diagnosis_score = evaluation_result.get('overall_performance')
                    if diagnosis_score is None:
                        diagnosis_score = evaluation_result.get('diagnosis_accuracy_score')
                    if diagnosis_score is None:
                        diagnosis_score = evaluation_result.get('overall_clinical_score')

                    # Format the score as a percentage if it's a number
                    score_display = None
                    if diagnosis_score is not None:
                        try:
                            # Format score based on scale (scores are out of 10)
                            if isinstance(diagnosis_score, (int, float)):
                                # Round to 1 decimal place and display as X/10
                                rounded_score = round(float(diagnosis_score), 1)
                                score_display = f"{rounded_score}/10"
                            else:
                                score_display = str(diagnosis_score)
                        except (ValueError, TypeError):
                            score_display = str(diagnosis_score)

                    recent_cases.append({
                        "id": str(eval.get('_id')),
                        "patientName": case_data.get('name', 'Unknown Patient'),
                        "age": case_data.get('age', '??'),
                        "gender": case_data.get('gender', '?')[0] if case_data.get('gender') else '?',
                        "specialty": case_data.get('specialty', 'General'),
                        "difficulty": case_data.get('difficulty', 'Unknown'),
                        "status": "Completed",
                        "date": eval.get('timestamp').strftime('%Y-%m-%d') if eval.get('timestamp') else 'Unknown',
                        "diagnosisCorrect": evaluation_result.get('diagnosis_correct', False),
                        "diagnosisScore": score_display,
                        "timeTaken": f"{round(eval.get('session_data', {}).get('efficiency_metrics', {}).get('total_time_mins', 0))} min"
                    })

                # Process in-progress cases from sessions
                for session in sessions:
                    ...
                    recent_cases.append({
                        ...
                        "timeTaken": f"{round(session.get('time_elapsed', 0) / 60)} min" if session.get('time_elapsed') else 'Unknown'
                    })

                # Sort combined cases by date (newest first) and limit to 10
                recent_cases.sort(key=lambda x: x['date'], reverse=True)
                dashboard_data["recentCases"] = recent_cases[:10]

                # Update total cases count (completed + in-progress)
                dashboard_data["stats"]["totalCases"] = dashboard_data["stats"]["completedCases"] + len(sessions)

                # Process specialty performance
                specialties = {}
                for eval in evaluations:
                    specialty = eval.get('case_info', {}).get('specialty', 'Unknown')
                    if specialty not in specialties:
                        specialties[specialty] = {"total": 0, "score_sum": 0}
                    ...
                    if score is not None and isinstance(score, (int, float)):
                        specialties[specialty]["score_sum"] += float(score)

                specialty_performance = []
                for specialty, data in specialties.items():
                    # Calculate average score for this specialty (out of 10)
                ...
                dashboard_data["specialtyPerformance"] = specialty_performance

                # Sample learning resources (in a real app, these would be personalized)
                dashboard_data["learningResources"] = [
                ...
                if not evaluations and not sessions:
                    ...
                    dashboard_data["specialtyPerformance"] = [
                        {"specialty": "Cardiology", "accuracy": 7.2},
                        {"specialty": "Neurology", "accuracy": 5.8},
                        {"specialty": "Pulmonology", "accuracy": 6.5},
                        {"specialty": "Infectious Disease", "accuracy": 4.2}
                    ]
